...
//
// Tests verifying Span positions (byte offset, line, column) for events
// produced from YAML input containing multi-byte UTF-8 characters.
//
// After Task C (lazy Span), Span carries byte offsets only. Line and column
// are resolved via LineIndex built from the input source.
//
...
use rlsp_yaml_parser::{Event, LineIndex, ScalarStyle, Span, parse_events};
...
fn assert_pos(label: &str, idx: &LineIndex, offset: u32, byte: u32, line: u32, col: u32) {
    assert_eq!(offset, byte, "{label}: byte_offset");
    let (got_line, got_col) = idx.line_column(offset);
    assert_eq!(got_line, line, "{label}: line");
    assert_eq!(got_col, col, "{label}: column");
}
...
/// Spike: plain scalar with ASCII content has correct `byte_offset`, line, column.
#[test]
fn plain_scalar_ascii_byte_offset_correct() {
    ...
    assert_pos("start", &idx, span.start, 0, 1, 0);
    assert_pos("end", &idx, span.end, 5, 1, 5);
}
...
fn mapping_key_ascii_byte_offset_correct() {
    ...
    assert_pos("start", &idx, span.start, 0, 1, 0);
    assert_pos("end", &idx, span.end, 3, 1, 3);
}
...
#[rstest]
#[case::multibyte_key("日本語: value\n", "日本語", (0u32, 1u32, 0u32), (9u32, 1u32, 3u32))]
...
fn scalar_span_start_and_end(
    #[case] input: &str,
    #[case] find_value: &str,
    #[case] start: (u32, u32, u32),
    #[case] end: (u32, u32, u32),
) {
    let events = collect_events(input);
    let idx = LineIndex::new(input);
    let (_, span) = events
    ...
    assert_pos("start", &idx, span.start, start.0, start.1, start.2);
    assert_pos("end", &idx, span.end, end.0, end.1, end.2);
}
...
fn multibyte_mapping_value_byte_offset_always_correct() {
    ...
    assert_eq!(span.start, 11, "byte_offset must be correct");
}
...
fn trailing_comment_ascii_all_fields_correct() {
    ...
    assert_pos("start", &idx, span.start, 6, 1, 6);
}
...
fn trailing_comment_after_multibyte_scalar_byte_offset_correct() {
    ...
    let (_, span) = events
        ...
        .find(|(ev, _)| matches!(ev, Event::Comment { .. }))
        .expect("Comment not found");
    assert_eq!(span.start, 10, "byte_offset of `#` must be 10");
}
...
fn trailing_comment_after_multibyte_scalar_column_correct() {
    ...
    assert_pos("scalar.start", &idx, scalar_span.start, 0, 1, 0);
    assert_pos("scalar.end", &idx, scalar_span.end, 9, 1, 3);
    ...
    let (_, comment_span) = events
        ...
        .find(|(ev, _)| matches!(ev, Event::Comment { .. }))
        .expect("Comment not found");
    assert_pos("comment.start", &idx, comment_span.start, 10, 1, 4);
    assert_pos("comment.end", &idx, comment_span.end, 16, 1, 10);
}
...
fn single_quoted_value_after_multibyte_key_column() {
    ...
    assert_pos("start", &idx, span.start, 5, 1, 3);
    assert_pos("end", &idx, span.end, 10, 1, 8);
}
...
fn block_literal_byte_offset_always_correct() {
    ...
    assert_eq!(span.start, 5, "byte_offset of `|` must be 5");
}
...
fn alias_span_with_multibyte_name_correct() {
    ...
    assert_pos("start", &idx, span.start, 16, 3, 0);
    assert_pos("end", &idx, span.end, 23, 3, 3);
}
...
fn multibyte_key_value_and_comment_positions() {
    ...
    let (_, key_span) = events
        ...
        .find(|(ev, _)| matches!(ev, Event::Scalar { value, .. } if value == "日本語"))
        .expect("Scalar(日本語) not found");
    assert_pos("key.start", &idx, key_span.start, 0, 1, 0);
    assert_pos("key.end", &idx, key_span.end, 9, 1, 3);
    ...
    let (_, val_span) = events
        ...
        .find(|(ev, _)| matches!(ev, Event::Scalar { value, .. } if value == "val"))
        .expect("Scalar(val) not found");
    assert_pos("val.start", &idx, val_span.start, 11, 1, 5);
    ...
    assert_pos("comment.start", &idx, comment_span.start, 15, 1, 9);
    assert_pos("comment.end", &idx, comment_span.end, 21, 1, 15);
}
