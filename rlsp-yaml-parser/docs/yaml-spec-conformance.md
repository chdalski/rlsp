...
- Implementation: Schema resolution is provided by `rlsp-yaml-parser/src/schema.rs` via `resolve_scalar` and `resolve_collection`. When `Schema::Failsafe` is active, `!`-tagged scalars resolve to `tag:yaml.org,2002:str`, `!`-tagged sequences resolve to `tag:yaml.org,2002:seq`, and `!`-tagged mappings resolve to `tag:yaml.org,2002:map`. The schema is applied in the loader (`rlsp-yaml-parser/src/loader.rs` — `Loader::load`) unconditionally for all three schemas. Select Failsafe via `LoaderBuilder::schema(Schema::Failsafe)`; the default is `Schema::Core`. All three schema paths resolve `!` non-specific tags.
...
- Implementation: The event stream represents unresolved nodes faithfully: untagged scalars, sequences, and mappings yield `tag: None` in `Event::Scalar`, `SequenceStart`, and `MappingStart`. The parser never synthesises a `?` tag token. The loader's schema resolution (Core by default) then assigns resolved tags in the AST — untagged scalars receive Core-pattern tags, untagged collections receive `!!map`/`!!seq`. Schema-resolved tags have `tag_loc: None` (no source position) to distinguish them from source-tagged nodes. Callers that need the raw `?`-tagged representation should use `parse_events()`, which is schema-agnostic.
- Test coverage: `rlsp-yaml-parser/tests/schema_resolution.rs` — `load_default_plain_integer_resolves_to_int`, `load_default_plain_null_resolves_to_null`, `load_default_mapping_resolves_to_map` (confirm AST tags after Core resolution). `rlsp-yaml-parser/tests/loader_tag_loc.rs` — `tl_4_tagless_scalar_has_tag_loc_none`, `tl_5_tagless_mapping_has_tag_loc_none`, `tl_6_tagless_sequence_has_tag_loc_none` (confirm schema-resolved nodes have `tag_loc: None`).
...
- Implementation: The `!!seq` shorthand is expanded by `DirectiveScope::resolve_tag` (`rlsp-yaml-parser/src/event_iter/directive_scope.rs:93–109`) to `"tag:yaml.org,2002:seq"`. The tag is stored on `SequenceStart` events and on `Node::Sequence { tag: Some("tag:yaml.org,2002:seq"), .. }` in the AST. Untagged sequences in the event stream have `tag: None`; the loader's Core schema resolution sets their AST tag to `tag:yaml.org,2002:seq`.
...
- Implementation: The `!!str` shorthand is expanded by `DirectiveScope::resolve_tag` (`rlsp-yaml-parser/src/event_iter/directive_scope.rs:93–109`) to `"tag:yaml.org,2002:str"`. The tag is stored on `Scalar` events and on `Node::Scalar { tag: Some("tag:yaml.org,2002:str"), .. }` in the AST. Untagged scalars in the event stream have `tag: None`; the loader's Core schema resolution assigns them a type-pattern tag (e.g. `tag:yaml.org,2002:str` for plain strings).
