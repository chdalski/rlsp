...
- [ ] Add detail text for scalar values
- [ ] Add label-key heuristic for sequence items
- [ ] Support non-mapping root documents
- [ ] Wrap multi-document files in document-level symbols

## Tasks

### Task 1: Add detail text for scalar values and label-key heuristic for sequence items
...
- [ ] `make_symbol` populates `detail` with truncated scalar value for `Scalar` nodes
- [ ] `make_symbol` populates `detail` with `"N keys"` for `Mapping` values and `"N items"` for `Sequence` values
- [ ] `make_sequence_children` uses label-key heuristic: checks first entry key of mapping items against `["name", "id", "key"]`, uses matching value as symbol name
- [ ] `make_sequence_children` falls back to `[i]` when no label key matches or item is not a mapping
- [ ] When label-key is used, `detail` on the sequence item shows the original index (e.g., `"[0]"`) so the position is not lost; when `[i]` is used as the name (fallback), `detail` is `None` (position is already visible in the name)
- [ ] Unit test: scalar value ≤60 chars appears verbatim in `detail`
- [ ] Unit test: scalar value >60 chars is truncated with `…` suffix in `detail`
- [ ] Unit test: mapping value shows `"N keys"` in `detail`; sequence value shows `"N items"` in `detail`
- [ ] Unit test: sequence item whose first key is `name` uses that key's scalar value as symbol name, not `[i]`
- [ ] Unit test: sequence item whose first key does not match `["name", "id", "key"]` falls back to `[i]`
- [ ] Unit test: sequence item whose first key is `name` but value is not a scalar falls back to `[i]`
- [ ] Integration test: send `textDocument/documentSymbol` for a Kubernetes-style YAML with `containers: [{name: nginx, ...}]` and verify `name` field is used as label and detail fields are populated
- [ ] All existing unit tests pass; tests that previously asserted `detail: None` are updated to assert the new expected value — no test may be deleted or disabled
- [ ] Tests that previously asserted `[0]`/`[1]` names on YAML whose first key matches the heuristic list (specifically `sequence_of_mappings_indexed_children_with_grandchildren` which uses `users: [{name: Alice, ...}]`) are updated to assert the label-key-derived name
