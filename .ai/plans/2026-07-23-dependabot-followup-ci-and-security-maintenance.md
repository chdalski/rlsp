...
**Runtime-path reachability finding (verified during
execution).** The plan premised the test-engineer
consultation on `brace-expansion` sitting on an *executed*
runtime path (`vscode-languageclient → minimatch@5.1.9`).
The test-engineer verified, and the lead independently
confirmed against `rlsp-yaml/src/server.rs:577-620`, that
this path is **architecturally unreachable** in the
extension today: the server advertises no
`diagnostic_provider`, `notebook_document_sync`, or
`workspace` file-operations capability (the only three that
make `vscode-languageclient` construct a `Minimatch`), and
the `documentSelector` carries no glob `pattern`. So no
integration test can exercise the glob path without changing
rlsp-yaml's `ServerCapabilities` — out of scope for an
overrides-only patch. This does **not** weaken the
remediation: Dependabot and `pnpm audit` flag a vulnerable
dependency's *presence* in the graph, not whether its code
runs, so the override still closes alert #37. Runtime-path
coverage is provided by a static lockfile-resolution
assertion instead (see the added test file below).

**Files correction.** This task's Files list originally named
only `package.json` + `pnpm-lock.yaml`, but the
test-engineer's input gate required a committed regression
test. The executed set therefore includes a third file,
`rlsp-yaml/integrations/vscode/src/overrides.test.ts` (the
lockfile-resolution assertion; imports only `fs`, `path`,
`vitest` — no new dependency). That is an expected advisor
deliverable, not scope creep.

- [x] In `rlsp-yaml/integrations/vscode/package.json`, add
...
- [x] In the same block, change `"fast-uri": "^3.1.2"` to
      `"fast-uri": "^3.1.4"`.
- [x] Run `pnpm install` in
      `rlsp-yaml/integrations/vscode` to regenerate
      `pnpm-lock.yaml`.
- [x] Verify every `brace-expansion` 2.x occurrence in the
      lockfile is `2.1.2` and none remains at `2.1.0`.
- [x] Verify every `brace-expansion` 5.x occurrence is
      unchanged at `5.0.7`.
- [x] Verify every `fast-uri` occurrence is `3.1.4` and none
      remains at `3.1.2`.
- [x] Run `pnpm audit` and report the full result. The only
      remaining finding must be `diff@7.0.0`
      (GHSA-73rr-hh4g-fpgx, low). Zero `brace-expansion` and
      zero `fast-uri` findings must remain.
- [x] Inspect the `pnpm-lock.yaml` diff and confirm it
...
- [x] Add a lockfile-resolution regression test
      (`src/overrides.test.ts`) asserting the overrides
      resolve on the correct graph nodes (per the
      test-engineer input gate; no new dependency).
- [x] Run the extension quality gates and confirm each
      passes: `pnpm run build`, `pnpm run lint`,
      `pnpm run format`, `pnpm run test`.
- [x] Run `xvfb-run -a pnpm run test:integration` and confirm
      it passes — as a general regression gate. (Per the
      reachability finding above, it does **not** exercise the
      `brace-expansion` glob path, which is unreachable.)
- [x] Confirm no scratch or temporary files remain in the
...
- `rlsp-yaml/integrations/vscode/src/overrides.test.ts` (new —
  test-engineer-required lockfile-resolution regression test)

**Result (2026-07-23):** overrides added
(`brace-expansion@2` → `^2.1.2`, `fast-uri` → `^3.1.4`, `@5`
untouched); lockfile regenerated — `brace-expansion` resolves
to `2.1.2` on both `minimatch@5.1.9` (runtime) and
`minimatch@9.0.9` (dev), `5.0.7` on the `@5` node,
`fast-uri` `3.1.4` everywhere; zero `2.1.0`, zero
`fast-uri@3.1.2`; lockfile diff 21 lines, package.json 3
lines, no unrelated churn. `pnpm audit` independently
reproduced by the reviewer and the lead: **3 high + 1 low →
1 low** (`diff@7.0.0`, the out-of-scope residual); zero
`brace-expansion`, zero `fast-uri`. Gates: vitest 43/43,
integration 19/19, build/lint/format clean,
`pnpm install --frozen-lockfile` in sync. All four advisor
gates cited and signed off.
