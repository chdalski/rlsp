...
mod flow_collections {
    ...
    // -----------------------------------------------------------------------
    // Group O: Reserved c-indicators inside flow collections must error
    // -----------------------------------------------------------------------

    #[test]
    fn anchor_indicator_in_flow_sequence_returns_error() {
        // `[&x]\n` — `&` is a reserved indicator inside flow context; not yet supported.
        let result: Vec<_> = parse_events("[&x]\n").collect();
        let has_error = result.iter().any(Result::is_err);
        assert!(
            has_error,
            "anchor indicator `&` inside flow collection must return an error"
        );
        // Iterator must stop after the first error.
        let mut saw_err = false;
        for item in &result {
            if saw_err {
                assert!(
    ...
    #[test]
    fn alias_indicator_in_flow_sequence_returns_error() {
        // `[*x]\n` — `*` is a reserved indicator inside flow context; not yet supported.
        let result: Vec<_> = parse_events("[*x]\n").collect();
        let has_error = result.iter().any(Result::is_err);
        assert!(
            has_error,
            "alias indicator `*` inside flow collection must return an error"
        );
        let mut saw_err = false;
        for item in &result {
            if saw_err {
