...
//!
//! Compares `rlsp-yaml-parser-temp::load()` against libfyaml's event API.
...
fn bench_throughput_by_size(c: &mut Criterion) {
    ...
    for (name, yaml) in cases {
        ...
        group.bench_with_input(BenchmarkId::new("load", name), yaml, |b, yaml| {
            b.iter(|| {
                let result = rlsp_yaml_parser_temp::load(black_box(yaml));
                black_box(result)
            });
        });
    }
    ...
    for (name, yaml) in cases {
        ...
        group.bench_with_input(BenchmarkId::new("parse_events", name), yaml, |b, yaml| {
            b.iter(|| {
                let count = rlsp_yaml_parser_temp::parse_events(black_box(yaml)).count();
                black_box(count)
            });
        });
    }
...
fn bench_throughput_by_style(c: &mut Criterion) {
    ...
    for (name, yaml) in cases {
        group.throughput(Throughput::Bytes(yaml.len() as u64));
        group.bench_with_input(BenchmarkId::new("load", name), yaml, |b, yaml| {
            b.iter(|| black_box(rlsp_yaml_parser_temp::load(black_box(yaml))));
        });
    }
    group.finish();
