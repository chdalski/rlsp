    ...
    for (name, text) in &sizes {
        group.bench_with_input(BenchmarkId::from_parameter(name), text, |b, text| {
            b.iter(|| {
                ...
                let _ = rlsp_yaml::validators::validate_custom_tags(text, docs, &allowed_tags);
                let _ = validate_key_ordering(text, docs);
...
fn bench_schema_validation(c: &mut Criterion) {
    ...
    let parsed: Vec<(&str, String, Vec<_>)> = sizes
        .iter()
        .map(|(name, text)| (*name, text.clone(), parse_yaml(text).documents))
        .collect();
