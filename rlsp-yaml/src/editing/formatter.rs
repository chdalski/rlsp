...
mod tests {
    ...
    // is itself a sequence item. Flow style is now preserved: output contains `[...]`.
    #[test]
    fn format_yaml_flow_sequence_in_mapping_in_sequence_item() {
        ...
        // Flow sequence is now preserved inline after `command: `.
        assert!(
            result.contains("command: ["),
            "flow sequence should be preserved inline: {result:?}"
        );
        assert!(
            result.contains('['),
    ...
    // Flow style is now preserved: output contains `[...]`.
    #[test]
    fn format_yaml_flow_sequence_in_nested_mapping() {
        ...
        // Flow sequence is now preserved inline.
        assert!(
            result.contains("command: ["),
            "flow sequence should be preserved inline: {result:?}"
        );
    ...
    // FI3: Single-element flow sequence in a mapping value — preserved as flow.
    #[test]
    fn format_yaml_single_element_flow_sequence() {
        ...
        // Flow sequence preserved inline: `args: ["--verbose"]`
        assert!(
            result.contains("args: ["),
            "flow sequence should appear inline after args: {result:?}"
        );
        assert!(
            result.contains("\"--verbose\""),
            "quoted flag should be preserved: {result:?}"
        );
    }
    ...
    // FI5: Top-level flow sequence is preserved as flow (not converted to block).
    #[test]
    fn format_yaml_top_level_flow_sequence_correct_indent() {
        ...
        // Flow style preserved: inline `[a, b, c]`.
        assert!(
    ...
    // ---- Group B: Flow sequence rendering ----

    // B1: Single-item flow sequence stays on one line.
    #[test]
    fn flow_sequence_single_item_stays_on_one_line() {
        let result = format_yaml("items: [a]\n", &default_opts());
        assert!(
            result.contains("[a]"),
            "single-item flow sequence missing: {result:?}"
        );
    }

    // B2: Multi-item flow sequence stays on one line when short.
    #[test]
    fn flow_sequence_multi_item_stays_on_one_line() {
        let result = format_yaml("items: [a, b, c]\n", &default_opts());
        assert!(
            result.contains("[a, b, c]"),
            "multi-item flow sequence missing: {result:?}"
        );
    }

    // B3: Long flow sequence breaks across lines when print_width is narrow.
    #[test]
    fn flow_sequence_long_breaks_across_lines() {
        let opts = YamlFormatOptions {
            print_width: 20,
            ..default_opts()
        };
        let input = "items: [alpha, bravo, charlie, delta, echo, foxtrot, golf, hotel]\n";
        let result = format_yaml(input, &opts);
        // The group must have broken — verify `]` is not on the same line as `[`
        let open_pos = result.find('[').expect("[ missing");
        let close_pos = result.find(']').expect("] missing");
        let between = &result[open_pos..=close_pos];
        assert!(
            between.contains('\n'),
            "long flow sequence should break across lines with narrow print_width: {result:?}"
        );
    }

    // B4: Flow sequence preserves item order.
    #[test]
    fn flow_sequence_preserves_item_order() {
        let result = format_yaml("nums: [3, 1, 4, 1, 5, 9]\n", &default_opts());
        assert!(
            result.contains("[3, 1, 4, 1, 5, 9]"),
            "item order wrong: {result:?}"
        );
    }

    // B5: Flow sequence with quoted items — brackets present, quoted item preserved.
    #[test]
    fn flow_sequence_with_quoted_items() {
        let result = format_yaml(
            "cmd: [\"python\", \"-m\", \"http.server\"]\n",
            &default_opts(),
        );
        assert!(result.contains('['), "flow bracket missing: {result:?}");
        // "-m" stays quoted (starts with '-')
        assert!(
            result.contains("\"-m\""),
            "quoted flag should be preserved: {result:?}"
        );
    ...
    // B6: Flow sequence is idempotent.
    #[test]
    fn flow_sequence_idempotent_simple() {
        let input = "items: [a, b, c]\n";
        let first = format_yaml(input, &default_opts());
        let second = format_yaml(&first, &default_opts());
        assert_eq!(first, second, "flow sequence not idempotent: {first:?}");
    }

    // B7: Long broken flow sequence is idempotent.
    #[test]
    fn flow_sequence_idempotent_long_broken() {
        let opts = YamlFormatOptions {
            print_width: 40,
            ..default_opts()
        };
        let input = "items: [alpha, bravo, charlie, delta, echo, foxtrot]\n";
        let first = format_yaml(input, &opts);
        let second = format_yaml(&first, &opts);
        assert_eq!(
            first, second,
            "broken flow sequence not idempotent: {first:?}"
        );
    }

    // ---- Group C: Flow mapping rendering ----

    // C1: Single-entry flow mapping with bracket_spacing: true gets spaces inside braces.
    #[test]
    fn flow_mapping_bracket_spacing_true() {
        let result = format_yaml("meta: {a: 1}\n", &default_opts());
        assert!(
            result.contains("{ a: 1 }"),
            "bracket spacing missing: {result:?}"
        );
    }

    // C2: Single-entry flow mapping with bracket_spacing: false — no spaces inside braces.
    #[test]
    fn flow_mapping_bracket_spacing_false() {
        let opts = YamlFormatOptions {
        ...
        assert!(
            result.contains("{a: 1}"),
            "expected no spaces in braces: {result:?}"
        );
        assert!(
            !result.contains("{ a: 1 }"),
            "unexpected spaces in braces: {result:?}"
        );
    }

    // C3: Multi-entry flow mapping with bracket_spacing: true.
    #[test]
    fn flow_mapping_multi_entry_bracket_spacing() {
        let result = format_yaml("meta: {a: 1, b: 2}\n", &default_opts());
        assert!(
            result.contains("{ a: 1, b: 2 }"),
            "multi-entry flow mapping missing: {result:?}"
        );
    }

    // C4: Long flow mapping breaks across lines when print_width is narrow.
    #[test]
    fn flow_mapping_long_breaks_across_lines() {
        let opts = YamlFormatOptions {
            print_width: 20,
            ..default_opts()
        };
        let input = "meta: {alpha: one, bravo: two, charlie: three, delta: four}\n";
        let result = format_yaml(input, &opts);
        assert!(
            result.contains('\n'),
            "result should have newlines: {result:?}"
        );
        // The block must span more than the key line — verify the mapping broke.
        let brace_open = result.find('{').expect("{ missing");
        let brace_close = result.find('}').expect("} missing");
        let inner = &result[brace_open..=brace_close];
        assert!(
            inner.contains('\n'),
            "long flow mapping should break across lines: {result:?}"
        );
    }

    // C5: Flow mapping is idempotent.
    #[test]
    fn flow_mapping_idempotent() {
        let input = "meta: {a: 1, b: 2}\n";
        let first = format_yaml(input, &default_opts());
        let second = format_yaml(&first, &default_opts());
        assert_eq!(first, second, "flow mapping not idempotent: {first:?}");
    }

    // C6: Flow mapping with string values.
    #[test]
    fn flow_mapping_with_string_values() {
        let result = format_yaml("labels: {app: web, env: prod}\n", &default_opts());
        assert!(result.contains("app: web"), "app: web missing: {result:?}");
        assert!(
            result.contains("env: prod"),
            "env: prod missing: {result:?}"
        );
        assert!(result.contains('{'), "opening brace missing: {result:?}");
        assert!(result.contains('}'), "closing brace missing: {result:?}");
    }

    // ---- Group D: Empty collections ----

    // D1: Empty flow sequence stays `[]`.
    #[test]
    fn empty_flow_sequence_stays_brackets() {
        let result = format_yaml("items: []\n", &default_opts());
        assert!(
            result.contains("[]"),
            "empty flow sequence should stay []: {result:?}"
        );
    }

    // D2: Empty flow mapping stays `{}`.
    #[test]
    fn empty_flow_mapping_stays_braces() {
        let result = format_yaml("status: {}\n", &default_opts());
        assert!(
            result.contains("{}"),
            "empty flow mapping should stay {{}}: {result:?}"
        );
    }

    // D3: Empty sequence value (parsed as flow) stays `[]`.
    #[test]
    fn empty_sequence_value_stays_brackets() {
        let result = format_yaml("empty_seq: []\n", &default_opts());
        assert!(
            result.contains("[]"),
            "empty sequence should format as []: {result:?}"
        );
    ...
    // ---- Group E: Mixed block/flow nesting ----

    // E1: Flow sequence nested inside block mapping — flow preserved.
    #[test]
    fn flow_sequence_inside_block_mapping_preserved() {
        let input = "config:\n  ports: [8080, 443]\n";
        let result = format_yaml(input, &default_opts());
        assert!(result.contains("config:"), "block key missing: {result:?}");
        assert!(
            result.contains("[8080, 443]"),
            "flow sequence should be preserved: {result:?}"
        );
    }

    // E3: Flow mapping nested inside block sequence — flow preserved.
    #[test]
    fn flow_mapping_inside_block_sequence_preserved() {
        let input = "items:\n  - {a: 1, b: 2}\n";
        let result = format_yaml(input, &default_opts());
        assert!(result.contains("items:"), "block key missing: {result:?}");
        assert!(
            result.contains('{'),
            "flow mapping should be preserved: {result:?}"
        );
        assert!(
            result.contains('}'),
            "flow mapping closing brace missing: {result:?}"
        );
    }

    // E4: Nested flow: flow sequence whose items are flow mappings.
    #[test]
    fn nested_flow_sequence_of_mappings() {
        let input = "data: [{a: 1}, {b: 2}]\n";
        let result = format_yaml(input, &default_opts());
        assert!(result.contains('['), "outer bracket missing: {result:?}");
        assert!(result.contains('{'), "inner brace missing: {result:?}");
    }

    // ---- Group F: format_enforce_block_style ----

    // F1: Flow sequence → block when enforce_block_style: true.
    #[test]
    fn enforce_block_style_converts_flow_sequence_to_block() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: true,
            ..default_opts()
        };
        let result = format_yaml("items: [a, b, c]\n", &opts);
        // Output must use block `- ` items, not `[...]`.
        assert!(
            !result.contains('['),
            "flow brackets must not appear: {result:?}"
        );
        assert!(result.contains("- a"), "block item missing: {result:?}");
        assert!(result.contains("- b"), "block item missing: {result:?}");
    }

    // F2: Flow mapping → block when enforce_block_style: true.
    #[test]
    fn enforce_block_style_converts_flow_mapping_to_block() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: true,
            ..default_opts()
        };
        let result = format_yaml("meta: {a: 1, b: 2}\n", &opts);
        // Non-empty flow mapping must be converted to block entries.
        assert!(
            !result.contains('{'),
            "flow brace must not appear: {result:?}"
        );
        assert!(result.contains("a: 1"), "block entry missing: {result:?}");
        assert!(result.contains("b: 2"), "block entry missing: {result:?}");
    }

    // F3: Block sequence stays block when enforce_block_style: true.
    #[test]
    fn enforce_block_style_block_sequence_stays_block() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: true,
            ..default_opts()
        };
        let result = format_yaml("items:\n  - a\n  - b\n", &opts);
        assert!(result.contains("- a"), "block item a missing: {result:?}");
        assert!(result.contains("- b"), "block item b missing: {result:?}");
    }

    // F4: Block mapping stays block when enforce_block_style: true.
    #[test]
    fn enforce_block_style_block_mapping_stays_block() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: true,
            ..default_opts()
        };
        let result = format_yaml("a: 1\nb: 2\n", &opts);
        assert!(result.contains("a: 1"), "block entry a missing: {result:?}");
        assert!(result.contains("b: 2"), "block entry b missing: {result:?}");
    }

    // F5: enforce_block_style: false (default) leaves flow intact.
    #[test]
    fn enforce_block_style_false_leaves_flow_intact() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: false,
            ..default_opts()
        };
        let result = format_yaml("items: [a, b]\n", &opts);
        assert!(
            result.contains('['),
            "flow bracket should be present: {result:?}"
        );
    }

    // ---- Group G: Idempotency ----

    // G1: Flow sequence idempotent — simple.
    #[test]
    fn flow_sequence_idempotent_g1() {
        let input = "items: [x, y]\n";
        let first = format_yaml(input, &default_opts());
        let second = format_yaml(&first, &default_opts());
        assert_eq!(first, second, "flow sequence not idempotent");
    }

    // G2: Flow mapping idempotent — simple.
    #[test]
    fn flow_mapping_idempotent_g2() {
        let input = "meta: {k: v}\n";
        let first = format_yaml(input, &default_opts());
        let second = format_yaml(&first, &default_opts());
        assert_eq!(first, second, "flow mapping not idempotent");
    }

    // G3: enforce_block_style: true is idempotent.
    #[test]
    fn enforce_block_style_idempotent() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: true,
            ..default_opts()
        };
        ...
        assert_eq!(
            first, second,
            "enforce_block_style not idempotent: {first:?}"
        );
    }

    // ---- Trailing comments adjacent to flow collections ----

    // Test 26: Trailing comment preserved when value is a flow sequence.
    #[test]
    fn flow_sequence_with_trailing_comment_preserved() {
        let input = "items: [a, b]  # my comment\n";
        let result = format_yaml(input, &default_opts());
        assert!(
            result.contains("# my comment"),
            "trailing comment missing: {result:?}"
        );
        // Comment must be on the same line as the flow sequence.
        for line in result.lines() {
            if line.contains('[') {
                assert!(
                    line.contains("# my comment"),
                    "comment not on same line as flow sequence: {line:?}"
                );
            }
        }
    }

    // Test 27: Trailing comment preserved when value is a flow mapping.
    #[test]
    fn flow_mapping_with_trailing_comment_preserved() {
        let input = "x: {a: 1}  # inline comment\n";
        let result = format_yaml(input, &default_opts());
        assert!(
    ...
    // Test 21 / Gap 2: enforce_block_style: true leaves empty flow map as `{}`.
    #[test]
    fn enforce_block_style_empty_flow_map_stays_empty() {
        let opts = YamlFormatOptions {
            format_enforce_block_style: true,
            ..default_opts()
        };
        ...
        assert_eq!(
            result, "status: {}\n",
            "empty flow map must stay {{}} with enforce_block_style: {result:?}"
        );
    }
}
