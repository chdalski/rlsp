...
// tests below only ever exercise these against whatever fast-uri/
...
// not vetted for each package, and the cross-contamination traps at the
// end: brace-expansion 3.1.6/4.1.3 are fast-uri's real floors, and
// fast-uri 2.1.4/5.0.9 are brace-expansion's real floors, so a table keyed
// by major only (dropping packageName) would wrongly flip every one of
// those four cases from false to true. There is no runtime "unknown
// package name" case here: GuardedPackage's literal union type makes that
// a compile error, not a reachable branch.
describe('isPatchedVersion', () => {
  it.each<[GuardedPackage, string, boolean]>([
    // fast-uri covers both patched floors and the two 4.x false-positive
    // traps (4.0.0 is major-4 but pre-floor; 4.1.2 is one patch below the
    // floor) that isAtLeast alone would miss.
    ...
    // Cross-contamination traps: fast-uri's real patched floors,
    // applied to brace-expansion, which has no major-3 floor at all and
    // a different (fully-vulnerable) major-4 status.
...
// onEmpty flag itself). Covers both packages' aggregation cases, plus two
// traps at the end that pass the *other* package's historical onEmpty
...
describe('allPatched', () => {
  it.each<[string[], GuardedPackage, boolean, boolean]>([
    // brace-expansion (onEmpty: false throughout).
    ...
    // fast-uri (onEmpty: true throughout).
    ...
    // onEmpty-flag traps: an empty array with the *other* package's
    // historical policy. If allPatched ignored onEmpty
