...
use shared::{
    CheckOutcome, collect_all_diagnostics, collect_corpus_files, collect_error_diagnostics,
    documents_equivalent, error_key, error_key_set, fmt_range, panic_message, run_check,
};
...
// ---------------------------------------------------------------------------
// Private helpers (I3 and I4 only)
// ---------------------------------------------------------------------------
...
fn missing_scalars(pre: &[String], post: &[String]) -> Vec<String> {
    let mut pre_counts: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
    for s in pre {
        *pre_counts.entry(s.as_str()).or_insert(0) += 1;
    }
    let mut post_counts: std::collections::HashMap<&str, usize> = std::collections::HashMap::new();
    for s in post {
...
mod tests {
    ...
    use super::*;
    use shared::helpers::{
        collect_from, load_docs, make_diag, make_doc, make_mapping, make_scalar, make_sequence,
        skip_list_contains, with_temp_dir, zero_span,
    };
